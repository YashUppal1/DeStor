...
contract('DeStor', ([deployer, uploader]) => {
  let destor

  before(async () => {
    destor = await DeStor.deployed()
  })

  describe('deployment', async () => {
    it('deploys successfully', async () => {
      const address = await destor.address
    ...
    it('has a name', async () => {
      const name = await destor.name()
      assert.equal(name, 'DeStor')
    })
  })
  ...
  describe('file', async () => {
    ...
    before(async () => {
      result = await destor.uploadFile(fileHash, fileSize, fileType, fileName, fileDescription, { from: uploader })
      fileCount = await destor.fileCount()
    })
    ...
    it('upload file', async () => {
      ...
      await destor.uploadFile('', fileSize, fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have size
      await destor.uploadFile(fileHash, '', fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await destor.uploadFile(fileHash, fileSize, '', fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await destor.uploadFile(fileHash, fileSize, fileType, '', fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have description
      await destor.uploadFile(fileHash, fileSize, fileType, fileName, '', { from: uploader }).should.be.rejected;
    })

    //check from Struct
    it('lists file', async () => {
      const file = await destor.files(fileCount)
